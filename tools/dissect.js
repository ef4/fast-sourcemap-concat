...
if (map.sourcesContent) {
  splitContents = map.sourcesContent.map(function (src) {
    return src ? src.split(newline) : [];
  });
} else {
...
for (var i = 0; i < lines.length; i++) {
  ...
  if (value.hasOwnProperty("originalLine")) {
    ...
    if (typeof origLine === "undefined") {
      origLine =
        "<BAD LINE REFERENCE: " + value.source + "," + value.originalLine + ">";
    }
  }
