...
SourceMap.prototype.addFile = function(filename) {
  ...
  if (url) {
    source = this._addMap(filename, url, source);
  } else {
...
SourceMap.prototype._addMap = function(filename, url, source) {
  var mappedLines = this._assimilateExistingMap(filename, url);
  var haveLines = countNewLines(source);
  while (mappedLines < haveLines) {
...
SourceMap.prototype._assimilateExistingMap = function(filename, url) {
  ...
  return this._scanMappings(srcMap, sourcesOffset, namesOffset);
};
...
SourceMap.prototype._scanMappings = function(srcMap, sourcesOffset, namesOffset) {
  ...
  while (inputMappings.length > 0) {
    ...
    if (!sourcesOffset && !namesOffset && (match = continuation.exec(inputMappings))) {
      ...
      decoder.resetColumn();
      lineCount += lines + match[1].replace(/,/g, '').length;
      mappings += match[0];
      inputMappings = inputMappings.slice(match[0].length);
    }
