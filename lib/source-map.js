...
SourceMap.prototype.addFile = function(filename) {
  ...
  this._cacheEncoderResults(hash(source), function(cacheHint){
    if (url) {
      source = this._addMap(filename, url, source, cacheHint);
    } else {
...
SourceMap.prototype._addMap = function(filename, url, source, cacheHint) {
  var initialLinesMapped = this.linesMapped;
  var haveLines = countNewLines(source);

  this._assimilateExistingMap(filename, url, cacheHint);
...
SourceMap.prototype._assimilateExistingMap = function(filename, url, cacheHint) {
