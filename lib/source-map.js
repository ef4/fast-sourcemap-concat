...
class SourceMap {
  constructor(opts) {
    ...
    if (opts.mapStyle === 'data' && (opts.mapFile || opts.mapURL)) {
      throw new Error("Must not specify mapStyle as 'data' when setting a custom mapFile");
    }
    ...
    this.mapFile = (this.mapStyle === 'file')
      ? opts.mapFile || (this.outputFile && this.outputFile.replace(/\.js$/, '') + '.map')
      : undefined;
  ...
  writeConcatStatsSync(outputPath, content) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      try {
        ...
        let sourceMappingUrl = sourceMap.mapURL;
        ...
        } else if (sourceMap.mapCommentType === 'hidden') {
          // sourceMaps are hidden
        } else {
          stream.write('/*# sourceMappingURL=' + sourceMappingUrl + ' */\n');
        }
